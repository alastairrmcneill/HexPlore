...
export async function listSharedAlbums(): Promise<SharedAlbum[]> {
  return PhotoLocationScanner.getSharedAlbums();
}

export async function scanSharedAlbums(
  albumIds: string[],
  onProgress: (processed: number, total: number) => void,
): Promise<{ hexCount: number; photoCount: number }> {
  onProgress(0, 1);
  const locations: PhotoLocation[] = await PhotoLocationScanner.getLocationsFromAlbums(albumIds);
